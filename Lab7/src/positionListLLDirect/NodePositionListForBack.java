 ...
 *
 */
public class NodePositionListForBack<T> implements PositionList<T> {

	private DNode<T> header, trailer; 
	private int count; 
	
	public NodePositionListForBack() {
	...
	public Iterable<Position<T>> positions() {
		NodePositionListForBack<Position<T>> pList = new NodePositionListForBack<Position<T>>(); 
		try { 
			if (!this.isEmpty()) { 
	...
	private static class DNode<T2> implements Position<T2> {
        ...
        private NodePositionListForBack<T2> myList;     // list it belongs to
        
        public DNode(T2 e, DNode<T2> p, DNode<T2> n, NodePositionListForBack<T2> list) { 
        	element = e; 
        	next = n; 
        	prev = p; 
        	myList = list; 
		...
		public void setList(NodePositionListForBack<T2> n) { 
			myList = n; 
		}
		public NodePositionListForBack<T2> getList() { 
			return myList; 
		}
